 ...
 * @property Ion_auth_system $ion_auth_system The Ion_auth_system library
 */
class Ion_auth_system_model extends CI_Model
{
	/**
	...
	public function __construct()
	{
		...
		$group_name = $this->config->item('database_group_name', 'ion_auth_system');
		if (empty($group_name))
		{
		...
		$this->tables = $this->config->item('tables', 'ion_auth_system');

		// initialize data
		$this->identity_column = $this->config->item('identity', 'ion_auth_system');
		$this->join = $this->config->item('join', 'ion_auth_system');

		// initialize hash method options (Bcrypt)
		$this->hash_method = $this->config->item('hash_method', 'ion_auth_system');

		// initialize messages and error
		$this->messages    = [];
		$this->errors      = [];
		$delimiters_source = $this->config->item('delimiters_source', 'ion_auth_system');
		...
		else
		{
			...
			$this->message_start_delimiter = $this->config->item('message_start_delimiter', 'ion_auth_system');
			$this->message_end_delimiter = $this->config->item('message_end_delimiter', 'ion_auth_system');
			$this->error_start_delimiter = $this->config->item('error_start_delimiter', 'ion_auth_system');
			$this->error_end_delimiter = $this->config->item('error_end_delimiter', 'ion_auth_system');
		}
	...
	public function register($identity, $password, $email, $additional_data = [], $groups = [])
	{
		...
		$manual_activation = $this->config->item('manual_activation', 'ion_auth_system');
		...
		else if (!$this->config->item('default_group', 'ion_auth_system') && empty($groups))
		{
		...
		$query = $this->db->get_where($this->tables['groups'], ['name' => $this->config->item('default_group', 'ion_auth_system')], 1)->row();
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->verify_password($password, $user->password, $identity))
			{
				...
				if ($this->config->item('remember_users', 'ion_auth_system'))
				{
					if ($remember)
					{
	...
	public function recheck_session()
	{
		...
		$recheck = (NULL !== $this->config->item('recheck_timer', 'ion_auth_system')) ? $this->config->item('recheck_timer', 'ion_auth_system') : 0;
	...
	{
		if ($this->config->item('track_login_attempts', 'ion_auth_system'))
		{
			$max_attempts = $this->config->item('maximum_login_attempts', 'ion_auth_system');
			if ($max_attempts > 0)
			{
	...
	public function get_attempts_num($identity, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth_system'))
		{
			...
			if ($this->config->item('track_login_ip_address', 'ion_auth_system'))
			{
				if (!isset($ip_address))
				{
					$ip_address = $this->input->ip_address();
			...
			$this->db->where('time >', time() - $this->config->item('lockout_time', 'ion_auth_system'), FALSE);
	...
	{
		if ($this->config->item('track_login_attempts', 'ion_auth_system') && $this->config->item('track_login_ip_address', 'ion_auth_system'))
		{
	...
	public function clear_login_attempts($identity, $old_attempts_expire_period = 86400, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth_system'))
		{
			...
			$old_attempts_expire_period = max($old_attempts_expire_period, $this->config->item('lockout_time', 'ion_auth_system'));
	...
	public function set_lang($lang = 'en')
	{
		...
		if($this->config->item('user_expire', 'ion_auth_system') === 0)
		{
		...
		{
			$expire = $this->config->item('user_expire', 'ion_auth_system');
		}
	...
	public function remember_user($identity)
	{
		...
		if ($token->validator_hashed)
		{
			...
			if ($this->db->affected_rows() > -1)
			{
				...
				set_cookie([
					'name'   => $this->config->item('remember_cookie_name', 'ion_auth_system'),
	...
	public function login_remembered_user()
	{
		...
		$remember_cookie = get_cookie($this->config->item('remember_cookie_name', 'ion_auth_system'));
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($this->verify_password($token->validator, $user->remember_code, $identity))
			{
				...
				if ($this->config->item('user_extend_on_login', 'ion_auth_system'))
				{
					$this->remember_user($identity);
				}
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = [])
	{
		...
		if ($this->config->item('admin_group', 'ion_auth_system') === $group->name && $group_name !== $group->name)
		{
	...
	public function delete_group($group_id = FALSE)
	{
		...
		if($group->name == $this->config->item('admin_group', 'ion_auth_system'))
		{
	...
	protected function _get_hash_parameters($identity = NULL)
	{
		...
		if ($identity)
		{
			...
			if ($user_id && $this->in_group($this->config->item('admin_group', 'ion_auth_system'), $user_id))
			{
				$is_admin = TRUE;
			}
		}
		...
		switch ($this->hash_method)
		{
			case 'bcrypt':
				$params = [
					'cost' => $is_admin ? $this->config->item('bcrypt_admin_cost', 'ion_auth_system')
										: $this->config->item('bcrypt_default_cost', 'ion_auth_system')
				];
				break;

			case 'argon2':
				$params = $is_admin ? $this->config->item('argon2_admin_params', 'ion_auth_system')
									: $this->config->item('argon2_default_params', 'ion_auth_system');
				break;
	...
	protected function _password_verify_sha1_legacy($identity, $password, $hashed_password_db)
	{
		...
		if ($this->config->item('store_salt', 'ion_auth_system'))
		{
		...
		else
		{
			...
			$salt_length = $this->config->item('salt_length', 'ion_auth_system');
	 ...
	 * @return array
	 */
	public function usersList($filter, $num = 0, $size = 0) {
		$this->db->reset_query();
	 ...
	 * @return array
	 */
	public function groupsList($filter, $num = 0, $size = 0) {
		$this->db->reset_query();
