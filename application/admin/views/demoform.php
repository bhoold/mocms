...
<div class="layui-form-item">
	...
	<div class="layui-input-block">
		<div class="layui-upload">
			<button type="button" class="layui-btn" lay-btn-event="showModelModal">选择图片</button>
...
<script>
layui.use(['form','layer','alert','laydate','util'], function(){
	var $ = layui.jquery,
		form = layui.form, //表单
	...
	util.event('lay-btn-event', {
		...
		showModelModal: function() {
			...
			$listElem.find('.item').each(function() {
				...
				var itemObj = {
					id: $(this).data('id'),
					src: $(this).find('.cover img').attr('src'),
			...
			layer.open({
				...
				yes: function(index, layero) {
					var isError = false,
						childFrame = $('iframe#layui-layer-iframe' + index)[0],
