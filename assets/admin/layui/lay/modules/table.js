      ...
      layui.each(data, function(i1, item1){
        ...
        that.eachCols(function(i3, item3){
          ...
          var td = ['<td data-field="'+ field +'" data-key="'+ key +'" '+ function(){ //追加各种属性
            var attr = [];
            if(item3.edit){ if(item1.edit === undefined){attr.push('data-edit="'+ item3.edit +'"');}else if(!(item1.edit === false)){attr.push('data-edit="'+ item1.edit +'"');} }  //是否允许单元格编辑
      ...
      return $.extend({
		...
		,add: function(newData){
			...
			if(newData){
				...
				that.eachCols(function(i, colItem){
					if(colItem.field){
						newRowData[colItem.field] = '';
					}
				});
			}
			data.splice(index+1, 0, newRowData);
			that.render();
		}
      }, sets);
    };
